...
let AuthService = class AuthService {
    ...
    async validateUser(userDto) {
        ...
        if (user && passwordEquals)
            return user;
        throw new exceptions_1.UnauthorizedException({ message: 'Неверный логин или пароль' });
    }
    async registration(registrationDto) {
        const candidate = await this.userService.getUserByEmail(registrationDto.email);
        if (candidate)
            throw new exceptions_1.HttpException('Пользователь с таким email уже существует', enums_1.HttpStatus.BAD_REQUEST);
